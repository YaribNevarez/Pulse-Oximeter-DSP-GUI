...
#define AUTOADJUST_TIME (500) // Milliseconds

#define TOP_SATURATION  (0x0700)
#define MIN_THRESHOLD   (0x0600)
#define DELTA           (0x00FF)
static void Poxi_adjust(void)
{
	...
	ADCTopSat = (redStat.maxInput.magnitude > TOP_SATURATION) || (iredStat.maxInput.magnitude > TOP_SATURATION);
	...
	if (ADCTopSat)
	{
		Poxi_DAQ.enviantLight += DELTA;
		if (0x0FFF < Poxi_DAQ.enviantLight)
		{
			Poxi_DAQ.enviantLight = DELTA;
			Poxi_DAQ.redLight  -= DELTA; // Intentional, possible underflow
			Poxi_DAQ.iredLight -= DELTA;
		}
		Poxi_DSP.red->resetStatistics(Poxi_DSP.red);
		Poxi_DSP.ired->resetStatistics(Poxi_DSP.ired);
	}
	else if (ADCMaxLevel < MIN_THRESHOLD)
	{
		Poxi_DAQ.enviantLight -= DELTA;
		if ((int16_t)Poxi_DAQ.enviantLight < 0)
		{
			Poxi_DAQ.enviantLight = 0;
			Poxi_DAQ.redLight  += DELTA; // Intentional, possible overflow
			Poxi_DAQ.iredLight += DELTA;
		}
		Poxi_DSP.red->resetStatistics(Poxi_DSP.red);
		Poxi_DSP.ired->resetStatistics(Poxi_DSP.ired);
